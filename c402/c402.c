...
**/
	...
	tBTNodePtr old_node = NULL;

	while (temp != NULL)
	{
		old_node = temp;

		if (temp->Cont == Content) // uz tam je, nic nebudeme delat
		{
			return;
		}

		if (temp->Cont > Content) // pujdeme doelava
		{
			temp = temp->LPtr;			
		}
		else // pujdmeme doprava
		{
			temp = temp->RPtr;
		}		
	}
	...
	else if (old_node->Cont > Content) // dame doleva
	{
		new_node->LPtr = NULL;
		new_node->RPtr = NULL;
		new_node->Cont = Content;
